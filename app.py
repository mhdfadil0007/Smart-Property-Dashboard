...
# -------------------
# Load & Clean Data (with 10 new rows added safely)
# -------------------
@st.cache_data
def load_properties():
    ...
    numeric_cols = ["Rent", "Area_In_Sqft", "Beds", "Baths"]
...
def compute_area_growth(transactions):
    ...
    for loc, grp in transactions.groupby("Area"):
        ...
        if len(grp_sorted) >= 2:
            ...
            growth_list.append({"Area": loc, "Growth %": round(growth_pct,2), "Avg Price": round(avg_price,2)})
...
with tab1:
    if location == "Select Location" or ptype == "Select Property Type":
        ...
        if not avg_rent_row.empty:
            ...
            fig_dist = px.histogram(avg_rent_row, x="Rent", nbins=30,
                                    title=f"Rent Distribution in {location}",
                                    color_discrete_sequence=["lightblue"],
                                    hover_data=["Beds", "Baths", "Type"])
            if rent > 0:
            ...
            type_counts = avg_rent_row["Type"].value_counts()
            fig_pie = px.pie(values=type_counts.values, names=type_counts.index,
                             title=f"Property Type Distribution in {location}")
            st.plotly_chart(fig_pie, use_container_width=True)
        else:
            st.info("No properties available for this location.")
    ...
    if not growth_df.empty:
        ...
        if len(loc_data) >= 2:
            fig_forecast = px.line(loc_data, x="year", y="Amount", markers=True,
                                   title=f"Transactions in {selected_area} with Forecast", hover_data=["Amount"])
...
with tab3:
    ...
    def compute_recommendations(df_props, df_growth, budget, req_type):
        df = df_props.copy()
        # --- FIX: Use Rent <= budget instead of Price ---
    ...
    if not matches.empty:
        ...
        st.dataframe(matches[["Location","Type","Beds","Baths","Rent","Rental Yield %", "Area Growth %","AI Score","Next Year Forecast","Recommendation","Insight"]].head(50))
        ...
        fig_radar = px.scatter(matches, x="Rental Yield %", y="AI Score", color="Recommendation",
                               size="Beds", hover_data=["Location","Type","Rent","AI Score","Area Growth %"],
...
with tab5:
    ...
    st.write("""
    Tools available:
    - Chatbot: Ask questions about average rents in locations.
    - SPA Upload: Extract structured details from your SPA document.
    - Search: Find properties in the dataset by ID or reference.
    """)
    ...
    if query:
        ...
        for loc in properties["Location"].unique():
            if loc.lower() in query.lower():
                ...
                st.write(f"Bot: Average rent in {loc} is {avg_rent:,.0f} AED.")
                answered = True
                break
        if not answered:
            st.write("Bot: This feature is in preview.")
    ...
    if uploaded:
        ...
        if "Property Id" in properties.columns:
            ...
            st.info("Property ID column not found in dataset. Search disabled.")
